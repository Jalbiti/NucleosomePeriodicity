# -*- coding: utf-8 -*-
"""Copy of nfr_clf.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Cbmv5deYOKJKBrJ5_VNVeCR2Qqrkay4B
"""
...
class Features:
  ...
  def featurize(self, testing_dataset=None):
    ...
    for i in range(len(self.chr_n)):
      ...
      # positions_chr = positions_chr.reset_index()
      ...
      # self.labels.append(["TSS"] * len(positions_chr))
      ...
      positions_chr = np.array(positions_chr)
      # positions_chr = np.array(positions_chr.reset_index()[0])
      # print(positions_chr)
...
"""
#Features
ene = np.loadtxt("NFR_pred/350data/def_energy_15k.txt", delimiter=' ')
tfbs = np.loadtxt("NFR_pred/350data/tfbs_15k.txt", delimiter=' ')
features = np.concatenate((ene, tfbs),axis=1)

model350.predict(features)

nfr_preds = []
for i in range(len(y_pred)):
  nfr_preds.append(y_pred[i][model350.encoder.transform(["NFR"])[0]])

nuc_preds = []
for i in range(len(y_pred)):
  nuc_preds.append(y_pred[i][model350.encoder.transform(["Nuc"])[0]])

chr = ["chrI"] * len(nfr_preds)
start = np.array(range(35000, 50001))
end = start + 1

d = {'chr': chr,'start': start, 'end': end, 'score': nfr_preds}
df = pd.DataFrame(data=d)
df.to_csv('nfr_preds.bed', index=False, sep='\t', header=False)

d = {'chr': chr,'start': start, 'end': end, 'score': nuc_preds}
df = pd.DataFrame(data=d)
df.to_csv('nuc_preds.bed', index=False, sep='\t', header=False)
"""
